...
#include "glcd_arduino_io.h"    // these macros map arduino pins & set CORE defines
...
#include "avrio.h"         // these macros do AVR direct port io    
#endif
...
#define glcdio_WriteByte(data) SR.SetData(data) 					\
// do {											\
 ...
 * Read the configured LCD data lines and return data as 8 bit byte
 */
#define glcdio_ReadByte() SR.ReadData()
// ({													\
// 	uint8_t rval = 0;										\
...
#define glcdio_SetRWDI(_RWstate, _DIstate) SR.SetRWDI(_RWstate, _DIstate)
// do {									\
 ...
 */
#define glcdio_RdBusystatus()	(SR.ReadData() & LCD_BUSY_FLAG)
//	(glcdio_ReadPin(GLCD_STATUS_BIT2PIN(LCD_BUSY_BIT)))
#define glcdio_RdResetstatus()	(SR.ReadData() & LCD_RESET_FLAG)
//	(glcdio_ReadPin(GLCD_STATUS_BIT2PIN(LCD_RESET_BIT)))
